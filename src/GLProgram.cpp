...
/*
void GLProgram::framebufferSizeCallback(GLFWwindow* window, int width, int
height) { glViewport(0, 0, width, height); windowWidth = width; windowHeight =
height;
}
*/

/*
void GLProgram::mouseButtonCallback(GLFWwindow* window, int button, int action,
int mods) {
...
/*
void GLProgram::scrollCallback(GLFWwindow* window, double xoffset, double
yoffset) { camera.processMouseScroll(yoffset);
}
*/
...
void GLProgram::cursorPosCallback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    if (mousePressed) {
        ...
        // to get the axis of rotation, need to convert from camera coordinates
to world coordinates glm::vec3 axisCamera = glm::cross(va, vb); glm::mat3
cameraToModel = glm::inverse(glm::mat3(camera.getViewMatrix() * modelMatrix));
...
glm::degrees(angleOfRotation), axisModel);
...
glm::vec3 GLProgram::getArcballVector(float x, float y) {
  ...
  // get normalized vector from center of the virtual arcball to a point P on
  // the arcball's surface if (x,y) is too far away from the arcball, return the
  // nearest point on the arcball's surface
